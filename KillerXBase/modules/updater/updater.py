...
@ren.on_message(filters.command("update", ".") & filters.me)
async def upstream(client: Client, message: Message):
        ...
        if changelog:
            ...
            if len(changelog_str) > 4096:
                ...
                await client.send_document(
                    message.chat.id,
                    "output.txt",
                    caption=f"**Type** `.update deploy` **To Update KillerXBase.**",
                    reply_to_message_id=status.id,
                )
                remove("output.txt")
            else:
                return await status.edit(
                    f"{changelog_str}\n**Type** `.update deploy` **To Update KillerXBase.**",
    ...
    if HEROKU_API_KEY is not None:
        ...
        if not HEROKU_APP_NAME:
            await status.edit(
                "`Please set up the HEROKU_APP_NAME variable to be able to update KillerXBase.`"
            )
            repo.__del__()
            return
        ...
        if heroku_app is None:
            await status.edit(
                f"{txt}\n`Invalid Heroku credentials for updating KillerXBase dyno.`"
            )
            repo.__del__()
            return
        await status.edit(
...
@ren.on_message(filters.command("goupdate", ".") & filters.me)
