...
@ren.on_message(filters.command("sudolist", ".") & filters.me)
...
@ren.on_message(filters.command("addsudo", ".") & filters.user(OWNER_ID))
...
@ren.on_message(filters.command("rmsudo", ".") & filters.user(OWNER_ID))
async def gmute_user(client: Client, message: Message):
    args = await extract_user(message)
    reply = message.reply_to_message
    ex = await message.reply_text("`Processing...`")
