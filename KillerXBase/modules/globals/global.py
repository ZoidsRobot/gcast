...
async def gban_user(client: Client, message: Message):
    ...
    if message.from_user.id != client.me.id:
        ...
        ex = await message.edit("`Whacking Gbanned....`")
    if not user_id:
    ...
    if (await KillerXBase.gban_info(user.id)):
    ...
    for gokid in f_chats:
        try:
            ...
            er += 1
    await KillerXBase.gban_user(user.id)
    ok.append(user.id)
    msg = (
        r"**\\#GBanned_User//**"
...
async def ungban_user(client: Client, message: Message):
    ...
    try:
        if not (await KillerXBase.gban_info(user.id)):
        ...
        for good_boi in ung_chats:
            try:
                ...
                er += 1
        await KillerXBase.ungban_user(user.id)
        msg = (
            r"**\\#UnGbanned_User//**"
            f"\n\n**First Name:** [{user.first_name}](tg://user?id={user.id})"
            f"\n**User ID:** `{user.id}`"
